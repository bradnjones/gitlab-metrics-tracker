...
describe('GitLabClient', () => {
  ...
  describe('fetchProject', () => {
    ...
    it('delegates to projectClient', async () => {
      const mockProject = { id: 'gid://gitlab/Project/123', name: 'Test Project' };
      client.projectClient.fetchProject = jest.fn().mockResolvedValue(mockProject);

      const result = await client.fetchProject();

      expect(client.projectClient.fetchProject).toHaveBeenCalledTimes(1);
      expect(result).toEqual(mockProject);
    });

    it('propagates errors from projectClient', async () => {
      client.projectClient.fetchProject = jest.fn().mockRejectedValue(new Error('fetch failed'));

      await expect(client.fetchProject()).rejects.toThrow('fetch failed');
    });
  });
